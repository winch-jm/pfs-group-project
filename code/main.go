...
func main(){
	...
	// Preprocess 
	// // - load dense rows
	file := "../data/threeline_subset_pca.csv"
	k := 15
	...
	// L
	P, levelStats := LeidenCommunityDetection(g, RBPM, 0.001, 0.3, 10, 10, 32)
	fmt.Println(len(P),P)
	fmt.Println(levelStats)

	g.WriteEdgeListCSV("../data/graph_edges_threeline.csv",true)
	// ... run leiden, get origP ...
	if err := WriteFinalPartitionCSV(sigIds, P, "../data/partition_threeline.csv"); err != nil {
		log.Fatal(err)
	}
